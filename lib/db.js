...
Db.prototype.addUrl = function(url, in_time, stock_code, site, type, log_obj, cb) {
    ...
    client.query('SELECT * FROM url WHERE url=?', [url], function(error, results, fields) {
        if (error) {
            cb(error, null);
            return;
        }
...
Db.prototype.addBulletin = function(url, stock_code, title, content, meta, in_time, log_obj, cb) {
    log_obj.log('6.1 开始检查公告是否已经存在。');
    client.query('SELECT id FROM bulletin WHERE url=?', [url], function(error, results, fields) {
        if (error) {
        ...
        if(results.length>0) {
            ...
            client.query('INSERT INTO bulletin SET site = ?, url = ?, title = ?, in_time = ?, stock_code = ?, content = ?, meta = ?', [ meta.site, url, title, in_time, stock_code, content, JSON.stringify(meta) ], function(err, results) {
                if (err) {
