...
Db.prototype.addArticleStockCode = function(article_id, stock_code, in_time, log_obj, cb) {
    ...
    client.query('SELECT id FROM article_stock WHERE article_id=? and stock_code=?', [article_id, stock_code], function(error, results, fields) {
        if (error) {
            log_obj.log('7.2 检查文章对应股票是否存在失败:'+error);
            cb(error);
            return;
        }
        ...
        //如果已经存在记录
        if(results.length>0) {
            log_obj.log('7.2 文章对应股票存在');
            cb('exists_stock', results[0].id);
            return;
        }
        else {
            log_obj.log('7.2 开始插入文章和股票对应关系。');
            client.query('INSERT INTO article_stock SET article_id=?, stock_code = ?, in_time = ?', [ article_id, stock_code, in_time ], function(err, results) {
                if (err) {
                    log_obj.log('7.3 文章对应股票添加失败'+err);
                    cb(err);
                }
                else {
                    log_obj.log('7.3 文章对应股票添加成功。');
