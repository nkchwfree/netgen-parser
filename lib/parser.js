        ...
        db.getInfoByPageId(url_id, function(error, info){
            ...
            workers.send({text:info.content, site:meta.site, type:meta.type, date:date, url:meta.url}, function(error, match){
                ...
                if(error) {
                    ...
                    log.debug(match);
                    var process = require("./process/"+meta.site+'.'+meta.type).process;
...
/*
                    ...
                    var addurl_function = function(url) {
                        db.addUrl(url, getTime(), info.stock_code, meta.site, match.url_list[i].type, log_obj, function(error, url_id){
                            if(!error) {
                                url_content.enqueue(getQueueUrl('url', url_id));
                                log_obj.log('5.0.插入URL成功:'+url);
                            }
                            else {
                                log_obj.log('5.0.插入URL失败:'+url + " " +error);
                            }
                        });
                    }
                    ...
                    for(var i=0;i<match.url_list.length;i++) {
                        addurl_function(match.url_list[i].url);
                    }
                    ...
                    if(match.bulletin_list && match.bulletin_list.length>0) {
                        for(var i=0;i<match.bulletin_list.length;i++) {
                            var flows = [getTask(match.bulletin_list[i].url, info.stock_code, match.bulletin_list[i].text, "", meta, getTime(), log_obj), db.addArticleStockCode, addStockQueue];
                            async.waterfall( flows, function(error){
                                if(error) {
