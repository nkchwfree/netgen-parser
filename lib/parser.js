...
var Parser = function(number) {
    ...
    //处理文章时，获得任务的方法。
    function getTask(url, stock_code, title, content, meta, time) {
        return function(callback) {
            db.addArticle(url, stock_code, title, content, meta, time, function(error, article_id){
                if(!error) {
                    article_content.enqueue(getQueueUrl('article_content', article_id));
                    //log.debug('Add Article:'+article_id);
                    callback(null, article_id, stock_code, time)
                }
                else {
                    callback(error);
                }
    ...
    function parsePage(task, cb) {
        ...
        log.debug(" TIME(before getInfoByPageId):"+task.uri);
        db.getInfoByPageId(url_id, function(error, info){
            log.debug(" TIME(begin getInfoByPageId):"+task.uri);
            if(error) {
            ...
            workers.send({text:info.content, site:meta.site, type:meta.type, date:date}, function(error, match){
                ...
                if(error) {
                    ...
                    if(match.content && match.content.length>0) {
                        ...
                        var flows = [getTask(meta.url, info.stock_code, match.title, match.content, meta, getTime()), db.addArticleStockCode, addStockQueue];
                    ...
                    if(match.bulletin_list && match.bulletin_list.length>0) {
                        for(var i=0;i<match.bulletin_list.length;i++) {
                            var flows = [getTask(meta.url, info.stock_code, match.bulletin_list[i].text, "", meta, getTime()), db.addArticleStockCode, addStockQueue];
                            async.waterfall( flows, function(error){
                                if(error) {
                                    log.log("Add Bulletin Error:"+error);
                                }
                            });
                        }
                    }
